    ...
    function(met_idx)
    {
      ...
      { . [ !(. %in%  met_args_name) ] } %>%
        ...
        {ifelse(test = all(.=='""'),
                yes = "",
                no =paste0(
                  "\n", met_name, " : argument ", ., " is missing, with no default"
                ))}
      ...
      msg_nui = sapply(
        ...
        function(nui)
        {
          ...
          { .[ !(. %in% nui_train_args_name) ] } %>%
            ...
            paste0('"', ., '"') %>%
            {
              ifelse(test = all(. == '""'),
                     yes = "",
                     no = paste0(
                       "\n ", names(met$nuisance_fct)[met$nuisance_fct %in% list(nui)],
          ...
          nui_pred_args_name = c("X", "Y", "Z", "trained.model") %>%
      ...
      if ((length(met$nuisance_fct) + 1) <= met$nbr_split) return(msg)
...
crossfit = function(X, Y, Z, Balancing_Method, default_nbr_crossfit=100,
                    default_nbr_fail=Inf, ...) {
    ...
    function(Bal_Met) {
      ...
      while (length(idx_met) > 0) {
        ...
        for (i in seq_along(idx_met)) {
          ...
          nbr_fail[j] = nbr_fail[j] + nbr_try - if (is.null(out_clean)) 0 else nrow(out_clean)
        }
